...
to .multiplai/inbox/ for review. Run /multiplai:process-learnings to apply.
...
Generated by AutoDream. Review with `/multiplai:process-learnings` to apply.
...
async def dream_report() -> None:
    ...
    print("Review with: /multiplai:process-learnings")
...
async def dream_auto() -> None:
    ...
    if not all_learnings:
        ...
        try:
            client = await create_client()
            logger.info("AutoDream (auto) using %s", type(client).__name__)
            ...
            if memory_files:
                ...
                logger.info("AutoDream (auto) complete: %d files updated", updated_count)
            else:
                logger.info("No memory files found to update")
        except Exception:
            logger.exception("AutoDream (auto) consolidation failed")
            raise
...
# Backward-compatibility alias — tests written against the original API call dream()
dream = dream_auto
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="AutoDream — learnings consolidation")
    parser.add_argument(
        "--check",
        action="store_true",
