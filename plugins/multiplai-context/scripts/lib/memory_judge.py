...
def item_key(item) -> str:
    ...
    judge is *shown* — target, section, change verb, text and the extractor's
    pair — and not on the proposal filename or the item's number: the same
    bullet re-drafted into a new proposal is the same question, and re-asking
    it would make two receipts disagree about one item.
    """
    parts = [
...
def parse_verdicts(raw: str) -> dict[tuple[str, int], Verdict]:
    ...
    A duplicate label keeps the **first** verdict. A model that answers twice
    for one item has already lost the thread, and taking the later line would
    let a trailing "summary" section overwrite real answers.
    """
