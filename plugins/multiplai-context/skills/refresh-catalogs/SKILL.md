...
2. **Invoke the catalog dispatcher:**
   Run `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py"` with the appropriate flags:
   - Default: `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py"`
   - Force: `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py" --force`
   - Dry-run: `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py" --dry-run`
   - Selective: `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py" --only memory,diary`
   - Combined: `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/generate_catalog.py" --force --dry-run`
...
- **Interpreter:** ALWAYS invoke via `uv run --project` exactly as shown above — `scripts/pyproject.toml` is a member of the repo-root uv workspace and declares the dependencies (`multiplai-core`, `claude-agent-sdk`) for every script in that directory. Bare `python`/`python3` fails with `ModuleNotFoundError: multiplai_core`. Do not switch this to `--no-project` with an inline PEP 723 header: that re-resolves the dependency on every run, which is what made the prompt hooks time out.
