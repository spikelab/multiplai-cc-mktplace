...
```sh
uv run --no-project ../../scripts/log_doctor.py --help
```
...
```sh
uv run --no-project ../../scripts/log_doctor.py --list --logs-dir "$(mktemp -d)" >/dev/null 2>&1 && echo list-ok
```
...
```sh
uv run --no-project ../../scripts/log_doctor.py --json --logs-dir "$(mktemp -d)" 2>/dev/null | python3 -c "import json,sys; json.load(sys.stdin); print('json-ok')"
```
