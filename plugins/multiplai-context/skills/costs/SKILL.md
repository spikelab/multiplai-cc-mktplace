...
1. **Collect first** so the report includes recent sessions:
   ```
   uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/collect_costs.py"
   ```
2. **Report** based on what the user asked:
   ...
   All via `uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/costs_report.py" ...`.
...
2. Report on it:
   ```
   uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/costs_report.py" --branch <that-branch>
   ```
...
- Records collected before this feature show `(none)` until the one-time
  enrichment is run:
  ```
  uv run --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/collect_costs.py" --backfill-branches
  ```
