...
1. **Check for pending learnings:**
   ```
   uv run --all-packages --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/dream.py" --check
   ```
...
```
uv run --all-packages --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/dream.py" --auto
```
...
- The default invocation (`uv run --all-packages --project "${CLAUDE_PLUGIN_ROOT}/../.." "${CLAUDE_PLUGIN_ROOT}/scripts/dream.py"`, no flags) is always report mode — safe to run anytime.
