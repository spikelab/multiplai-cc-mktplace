...
2. **Dry-run first** to see what will be processed:
   ...
   ```
   uv run --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/backfill.py" --dry-run [--days N] [--projects slug,...]
   ```
...
4. **Run the backfill** — long-running (minutes to hours, well past the Bash tool's
   ...
   ```
   uv run --no-project "${CLAUDE_PLUGIN_ROOT}/scripts/backfill.py" [--days N] [--since DATE] [--all]
