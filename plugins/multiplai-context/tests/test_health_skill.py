"""Tests for the /multiplai-context:health skill — memory audit.
...
class TestHealthSkillPrompt:
    ...
    def test_recommends_setup_for_missing(self):
        """Health skill must recommend /multiplai-context:setup for missing config."""
...
class TestHealthCheckMemoryInventory:
    ...
    def test_required_missing_drives_setup_not_arbitrary_files(self):
        ...
        assert "required_missing" in self.source
        assert re.search(
            r"required_missing.*\n.*multiplai-context:setup|setup.*required",
            self.source, re.DOTALL,
        ) or "Run /multiplai-context:setup" in self.source
...
class TestHealthCheckStalenessDetection:
    ...
    def test_recommends_dream_for_stale(self):
        """health_check.py must recommend /multiplai-context:dream for stale files."""
...
class TestHealthCheckFreshInstall:
    ...
    def test_recommends_setup(self):
        """health_check.py must recommend running /multiplai-context:setup."""
