...
Line numbers were re-derived on **2026-08-03** against the buildme subtree in
...
The eleven names are an enum — `models.py:58 BoardColumn`, values
`models.py:70-80` — so the same string appears in `.board.json`'s `column` field
...
`board.py:112 column_for(phase, block_status)` is the only mapping, and it is a
pure function over the table at `board.py:90 _PHASE_COLUMNS` (exhaustive over
`BuildPhase`). Cards are written by `board.py:186 record` / `board.py:214 _record`.

| # | Column | Covered? | Evidence |
|---|--------|----------|----------|
| 1 | Backlog | **Never driven.** No phase maps to it; nothing in the pipeline writes it. | Absent from `board.py:90-108 _PHASE_COLUMNS`; stated in the module docstring `board.py:40-45` |
| 2 | Accepted | **Mapped, never written.** `BuildPhase.INIT` maps to Accepted, but no card is recorded that early — the change directory does not exist yet at INIT, so the first recorded column is always Shaping. | `board.py:91` (INIT → ACCEPTED); `board.py:29-33`; first actual write is `orchestrator.py:80` at BOOTSTRAP |
| 3 | Shaping | **Driven.** BOOTSTRAP, INTERVIEW_DONE, RESEARCH, CODEBASE_ANALYSIS and SPEC_GENERATION all map to Shaping and each records the card. Products: `codebase-analysis.md`, `proposal.md`, `requirements/*.md`. | `board.py:92-98`; recorded at `orchestrator.py:80`, `:108`, `:129`, `:142`, `:167` |
| 4 | Planning | **Driven.** DESIGN_AUDIT, PROTOTYPE and REVIEW map to Planning. The "reviewed by another eng" half of the column is stood in for by the adversarial design audit and the tasks-shape audit, plus the prototype's findings pass. | `board.py:99-101`; recorded at `orchestrator.py:181`, `:189`, `:209`; audits at `llm_steps/spec_steps.py:220 run_design_audit`, `spec_generator.py:509 _audit_tasks_shape`; prototype at `orchestrator.py:185-189` → `llm_steps/prototype_steps.py:36 run_prototype` |
| 5 | In Development | **Driven.** TDD_BUILD and RESPEC map to In Development, and the card is recorded **before** the engine runs, so a card is in development for the whole build rather than only once it succeeds. Every block-status change re-records the same column. | `board.py:102-103`; recorded at `orchestrator.py:216` (pre-engine) and `:261`; per-block at `tdd_engine.py:80` |
| 6 | In Review | **Driven, but only on a real push + PR.** `BuildPhase.PUBLISH` itself still maps to In Development; In Review is recorded from the publish step *after* `git push` succeeded **and** `gh pr create` returned. A run with `--no-push`, `--no-pr`, `--no-worktree`, or a failed push finishes in In Development. Its exit condition (merged to staging) is **not** implemented. | `board.py:104-106` (PUBLISH → IN_DEVELOPMENT); push at `orchestrator.py:834`, PR at `:851`, In Review recorded at `orchestrator.py:875-879`; publish skipped with no pipeline branch at `orchestrator.py:806-808` |
| 7 | Testing | **Never driven.** There is no staging deploy and no QA/E2E agent. The nearest thing is a post-TDD entry-point smoke check, which warns and does not move any card. | `board.py:40-45`; `tdd_engine.py:1848 _verify_entry_point`, called at `tdd_engine.py:1688`, records no board column |
| 8 | Ready for Prod | **Never driven.** No prod-PR automation exists. | `board.py:40-45`; no `BoardColumn.READY_FOR_PROD` outside `models.py:77` and `board.py:82` |
| 9 | Deploying | **Never driven.** No deploy machinery, by design. | `board.py:40-45` |
| 10 | Deployed | **Never driven.** Same. | `board.py:40-45` |
| 11 | Cancelled | **Driven, narrowly.** Recorded only when the run ended unrecoverably — defined as "no resumable checkpoint survives" (`.build-state.json` gone). An ordinary failed phase leaves the checkpoint, so the card stays in its last column and a resume continues from it. | `board.py:269 record_failure` (guard at `:281-288`); callers `orchestrator.py:226`, `:344`; `board.py:108` (FAILED → CANCELLED) |

### `BlockStatus` moves no column

`column_for` accepts a `BlockStatus` and ignores it — every block state lives
inside In Development (`board.py:112-119`). In particular
...
(`tdd_engine.py:1472 _run_quality_review`, `tdd_engine.py:1767 _run_final_review`).
...
(`orchestrator.py:834`, `:851`) — a reviewer *can* `git fetch` and diff it.
...
`tdd_engine.py:1848 _verify_entry_point`, which asks whether the entry point
...
(`models.py:78-79`) purely so a later scheduler has the vocabulary, and
`board.py`'s docstring records that the pipeline must not pretend to move a card
into them (`board.py:40-45`). Anything that deploys is a separate system with its
...
- **`.board.json`** — written to the change directory
  (`board.py:70 BOARD_FILENAME`, path resolved by `board.py:152 board_path`).
  Schema: `board.py:139 BoardCard` — `card_id`, `change_name`, `column`,
  `owner_agent`, `entered_at`, `branch`, `worktree_path`, `pr_url`, and a
  `history` of `{column, at, note}` events (`board.py:133 BoardEvent`).
- **`BOARD:<change>:<Column>`** on stdout — emitted only when the card actually
  moves columns (`board.py:255-259`), alongside the existing `PHASE:` protocol.
- **Owner seats** — `board.py:74 _OWNERS` names the seat per column (product,
...
- **Card identity** — `board.py:127 card_id_for` returns `buildme-<normalized
...
- **Never raises** — a board write failure logs and lets the build continue
  (`board.py:204`); a corrupt card is ignored (`board.py:175 read_card`).

### Honest caveats

- In `--auto` runs the archive move happens *before* PUBLISH
  (`orchestrator.py:272-294`), so the final In Review card is written into
  `specs/archive/<date>-<name>/.board.json` (`board.py:152-172`) and — because
...
- `pr_url` is recorded from the **live** `BuildState`, never re-read from
  ...
  (`board.py:199-202`, `orchestrator.py:294`).
