...
The eleven names are an enum — `models.py:50 BoardColumn`, values
`models.py:62-72` — so the same string appears in `.board.json`'s `column` field
...
`board.py:105 column_for(phase, block_status)` is the only mapping, and it is a
pure function over the table at `board.py:86 _PHASE_COLUMNS` (exhaustive over
`BuildPhase`). Cards are written by `board.py:177 record` / `board.py:205 _record`.

| # | Column | Covered? | Evidence |
|---|--------|----------|----------|
| 1 | Backlog | **Never driven.** No phase maps to it; nothing in the pipeline writes it. | Absent from `board.py:86-102 _PHASE_COLUMNS`; stated in the module docstring `board.py:37-42` |
...
| 9 | Deploying | **Never driven.** No deploy machinery, by design. | `board.py:37-42` |
| 10 | Deployed | **Never driven.** Same. | `board.py:37-42` |
| 11 | Cancelled | **Driven, narrowly.** Recorded only when the run ended unrecoverably — defined as "no resumable checkpoint survives" (`.build-state.json` gone). An ordinary failed phase leaves the checkpoint, so the card stays in its last column and a resume continues from it. | `board.py:245 record_failure` (guard at `:253-260`); callers `orchestrator.py:206`, `:298`; `board.py:101` (FAILED → CANCELLED) |

### `BlockStatus` moves no column

`column_for` accepts a `BlockStatus` and ignores it — every block state lives
inside In Development (`board.py:105-112`). In particular
...
(`tdd_engine.py:1154 _run_quality_review`, `tdd_engine.py:1379 _run_final_review`).
...
(`orchestrator.py:616`, `:633`) — a reviewer *can* `git fetch` and diff it.
Nothing does. The missing pieces:

- **A reviewer agent** that fetches the pushed branch, diffs it against the base,
  reads the change's `specs/` (proposal, requirements, design, `unknowns.md`,
  `respec.md`) and reviews the diff against them — a real out-of-process review,
...
- **Staging-merge automation** to satisfy the column's stated exit condition
  ...
  `git_ops.py:279 remove_worktree` is a documented helper deliberately never
...
`tdd_engine.py:1457 _verify_entry_point`, which asks whether the entry point
...
(`models.py:70-71`) purely so a later scheduler has the vocabulary, and
`board.py`'s docstring records that the pipeline must not pretend to move a card
into them (`board.py:37-42`). Anything that deploys is a separate system with its
...
- **`.board.json`** — written to the change directory
  (`board.py:66 BOARD_FILENAME`, path resolved by `board.py:145 board_path`).
  Schema: `board.py:132 BoardCard` — `card_id`, `change_name`, `column`,
  `owner_agent`, `entered_at`, `branch`, `worktree_path`, `pr_url`, and a
  `history` of `{column, at, note}` events (`board.py:126 BoardEvent`).
- **`BOARD:<change>:<Column>`** on stdout — emitted only when the card actually
  moves columns (`board.py:232-235`), alongside the existing `PHASE:` protocol.
- **Owner seats** — `board.py:70 _OWNERS` names the seat per column (product,
...
- **Card identity** — `board.py:120 card_id_for` returns `buildme-<normalized
...
- **Never raises** — a board write failure logs and lets the build continue
  (`board.py:198-202`); a corrupt card is ignored (`board.py:166 read_card`).

### Honest caveats

- In `--auto` runs the archive move happens *before* PUBLISH
  (`orchestrator.py:243-263`), so the final In Review card is written into
  `specs/archive/<date>-<name>/.board.json` (`board.py:145-163`) and — because
...
- `pr_url` is recorded from the **live** `BuildState`, never re-read from
  ...
  (`board.py:190-193`, `orchestrator.py:255`).
