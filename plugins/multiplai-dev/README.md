...
| `review` | Code review as a pipeline of separate agents (find, verify, prescribe, check) with gates in code that reject any finding or fix whose cited lines are not at the reviewed commit; writes a markdown review, rollups and a `findings.json`, then opens it in `review-viewer`. |
