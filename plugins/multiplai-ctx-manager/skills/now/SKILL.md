...
e.g. after a `/multiplai:backfill`, after editing `project-map.yaml`, or to
...
- **Full rebuild (all projects):**
  ```
  python "${CLAUDE_PLUGIN_ROOT}/scripts/synthesize_now.py"
  ```
- **Single project:**
  ```
  python "${CLAUDE_PLUGIN_ROOT}/scripts/synthesize_now.py" --project <name>
  ```
...
- Project names come from `.multiplai/project-map.yaml`. If snapshots are landing
  ...
  config, not here — point the user at `/multiplai:setup` to revise it.
