...
2. **Parse the JSON output** and present a markdown-formatted audit report with these sections:
   ...
   - **Routing Quality** (the single most critical signal — lead with it): parse the `routing` block.
     - **Strategy**: report `configured_strategy` vs `effective_strategy`. If `degraded_to_fallback` is true, state plainly: *the semantic `llm` router is configured but no model client is available, so routing has degraded to the offline `token_overlap` fallback.* Recommend ensuring the Agent SDK (run inside Claude Code) or an API key so the default semantic router actually runs.
     - **Live aggregates** (`routing.live`, from recent real prompts): report `samples`, `cap_saturation_pct`, `mean_picked`, and the `mean_top_score`→`mean_floor_score` spread. Interpret honestly: high saturation (>50%) with a flat spread (top ≈ floor) = low-signal routing (the cutoff is arbitrary). A healthy `token_overlap` fallback shows saturation well under 20% and top noticeably above floor. Few samples (<10) → say it's not yet meaningful.
     - **Last offline eval** (`routing.last_eval`): report `strategy`, `total_cases`, `recall_pct`, `precision_pct`, `none_accuracy_pct`, `cap_saturation_pct`, and `age_days`. **Mandatory caveats — state these, do not present the numbers bare:**
       - `token_overlap` `none_accuracy_pct ≈ 0` is **expected and by design**, not a regression — lexical overlap structurally cannot abstain (proven: NONE and true-positive score distributions fully overlap). That is precisely why `llm` is the default; `token_overlap` is only the offline fallback.
       - The offline eval **can only score `token_overlap`**. If `last_eval.strategy` is `token_overlap` while `effective_strategy` is `llm`, the eval numbers do **not** describe what's actually running — say so. Scoring the `llm` default requires an API key (or an in-session harness); flag this as a known gap.
